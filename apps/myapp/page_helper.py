...
# -------------------------------------pageinfo_search_by_time--------------------------------------------------------------------------------------
class pageinfo_search_by_time(object):
    def __init__(self, page, count, perItem, start_time, end_time):
        self.__page = page
        self.__count = count
...
def pager_oplog_list_search(request, page, pageCount, start_time, end_time):
    page_html = []
    first_html = '<a href="/cmdb/index/audit/oplog/search_result/start_time=%s&end_time=%s&page=%d">首页</a>' % (start_time, end_time, 1,)
    page_html.append(first_html)
    if page == 1:
        pre_html = '<a href="/cmdb/index/audit/oplog/search_result/start_time=%s&end_time=%s&page=%d">上页</a>' % (start_time, end_time, 1,)
    else:
        pre_html = '<a href="/cmdb/index/audit/oplog/search_result/start_time=%s&end_time=%s&page=%d">上页</a>' % (start_time, end_time, page - 1,)
    ...
    for i in range(pageStart, pageEnd):
        if page == i + 1:
            a_html = '<a style="color:red;"href="/cmdb/index/audit/oplog/search_result/start_time=%s&end_time=%s&page=%d">[%d]</a>' % (
                start_time, end_time, i + 1, i + 1)
        else:
            a_html = '<a href="/cmdb/index/audit/oplog/search_result/start_time=%s&end_time=%s&page=%d">[%d]</a>' % (
                start_time, end_time, i + 1, i + 1)
        page_html.append(a_html)
    if page == pageCount:
        next_html = '<a href="/cmdb/index/audit/oplog/search_result/start_time=%s&end_time=%s&page=%d">下页</a>' % (start_time, end_time, pageCount,)
    else:
        next_html = '<a href="/cmdb/index/audit/oplog/search_result/start_time=%s&end_time=%s&page=%d">下页</a>' % (start_time, end_time, page + 1,)
    page_html.append(next_html)
    end_html = '<a href="/cmdb/index/audit/oplog/search_result/start_time=%s&end_time=%s&page=%d">尾页</a>' % (start_time, end_time, pageCount,)
