    ...
    if request.method == 'POST':
        ...
        if not (is_exist):
            ...
            if is_empty:
                ...
                #queryset.group.set(group)
                queryset = userInfo.objects.create(username=username,email=email,first_name=first_name,last_name=last_name,
...
def userDetail(request, *args, **kwargs):
        ...
        msg = {'username':username,'first_name':first_name,'last_name':last_name, 'email':email,'is_staff':is_staff,'is_active':is_active,
               'date_joined':date_joined,'workflow_order':workflow_order,'password':password,'last_login':last_login,'is_superuser':is_superuser,
