...
from apps.myapp import account, apps
...
urlpatterns = [
    ...
    url('^register/active/token=(?P<token>\w+)', account.active),
    ...
    url('^index/table/user/search/', account.search),
    url('^index/table/user/search_result/keyword=(?P<keyword>\w+)&page=(?P<page>\d+)', account.search_result),
